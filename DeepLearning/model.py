...
from DeepLearning.DeepLearningHandler import connectionTransformation

class Model(object):

    def __init__(self, t_layer_sizes, p_layer_sizes,lower,upper, dropout=0):
        ...
        self.t_input_size = abs(upper - lower)
        ...
        self.srng = T.shared_randomstreams.RandomStreams(np.random.randint(0, 1024)) # random seed
        self.setup_train()
        self.setupPrediction()
        self.setupSlowFunction()
    ...
    def setupPrediction(self):
        ...
        def step_time(*states):
            ...
            notes_result, updates = theano.scan(fn=self._predictStepNote, sequences=[time_final], outputs_info=note_outputs_info)

            # Now notes_result is a list of matrix [layer/output](notes, onOrArtic)
            output = get_last_layer(notes_result)

            next_input = connectionTransformation()(output, time + 1) # TODO: Fix time
    ...
    def setupSlowFunction(self):
        ...
        note_outputs_info = ([ initial_state_with_taps(layer) for layer in self.pitch_model.layers ] +
                             [ dict(initial=start_note_values, taps=[-1]) ])
        notes_result, updates = theano.scan(fn=self._predictStepNote, sequences=[time_final], outputs_info=note_outputs_info)
        # Now notes_result is a list of matrix
        output = get_last_layer(notes_result)
        next_input = connectionTransformation()(output, self.walk_time + 1)
        slowFunctionRes = (new_states[:-1] + notes_result[:-1] + [ next_input, output ])
        updates.update({
        ...
        self.slowFunction = theano.function(
            inputs=[self.conservativity],
            outputs=slowFunctionRes,
            updates=updates,
            allow_input_downcast=True)

    def initSlowLearning(self, seed):
        seed = np.array(seed)
